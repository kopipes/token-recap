document.addEventListener('DOMContentLoaded', () => {
    ...
    const idrRateInput = document.getElementById('idr-rate');

    // IDR rate — load from localStorage or default 26000
    // GCP uses bank rate + markup, typically ~Rp26,000-27,000/USD (not the mid-market rate)
    let idrRate = parseInt(localStorage.getItem('idrRate') || '26000', 10);
    if (idrRateInput) {
        idrRateInput.value = idrRate;
        idrRateInput.addEventListener('change', () => {
            idrRate = parseInt(idrRateInput.value, 10) || 16000;
    ...
    /**
     * Calculate cost with modality awareness.
     * Thinking tokens (text_thinking) have a higher output price than regular text.
     * Audio/image/video input tokens have their own pricing tiers.
     */
    const calculateCost = (modelName, inputTokens, outputTokens, modality = null) => {
        ...
        let inputPrice = prices.input;
        let outputPrice = prices.output;

        // Note: thinking tokens (text_thinking) use the same base price as regular output.
        // GCP does NOT charge a separate "thinking" surcharge on the token count metric —
        // thinking tokens are already included in the output token count at the standard rate.
        // Audio input tokens are priced differently (approx 40× text input per token).
        if (modality) {
            const m = modality.toLowerCase();
            if (m === 'audio' || m === 'audio_cached') {
                inputPrice = inputPrice * 40;
            }
        }
    ...
    const fetchData = async () => {
        try {
            tableBody.innerHTML = '<tr><td colspan="9" class="text-center loading-text">Loading data...</td></tr>';
            ...
            const params = new URLSearchParams({
                ...
                modality: modalityFilter ? modalityFilter.value : 'all',
    ...
    // Format IDR with Rp prefix and thousands separator
    const formatIDR = (usd) => {
        const idr = Math.round(usd * idrRate);
    ...
    const renderTable = (data) => {
        if (data.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="9" class="text-center loading-text">No usage data found for the selected period.</td></tr>';
            updatePagination(0);
            return;
        }
        ...
        // Compute grand totals from ALL data (not just current page)
        data.forEach(item => {
            const inputT = parseInt(item.input_tokens || 0, 10);
            const outputT = parseInt(item.output_tokens || 0, 10);
            grandTotalTokens += parseInt(item.total_tokens, 10);
            grandTotalInput += inputT;
            grandTotalOutput += outputT;
            grandTotalCost += calculateCost(item.model, inputT, outputT, item.modality);
        });
        ...
        pageData.forEach(item => {
            ...
            const inputT = parseInt(item.input_tokens || 0, 10);
            const outputT = parseInt(item.output_tokens || 0, 10);
            const cost = calculateCost(item.model, inputT, outputT, item.modality);
            ...
            let modalityColor = '#8b949e';
            if (modality === 'text_thinking') modalityColor = '#a371f7';
            else if (modality === 'text') modalityColor = '#3fb950';
            else if (modality === 'audio') modalityColor = '#f0883e';
            else if (modality === 'image') modalityColor = '#58a6ff';
            else if (modality === 'video') modalityColor = '#d29922';
            ...
            tr.innerHTML = `
                ...
                <td><span style="background: rgba(255,255,255,0.05); color: ${modalityColor}; padding: 0.2rem 0.6rem; border-radius: 4px; font-size: 0.8rem; font-weight: 600; border: 1px solid ${modalityColor}40;">${modality}</span></td>
                <td class="text-right font-medium">${total.toLocaleString()}</td>
                <td class="text-right" style="color: #8b949e;">${inputT.toLocaleString()}</td>
                <td class="text-right" style="color: #8b949e;">${outputT.toLocaleString()}</td>
                <td class="text-right" style="color: #3fb950; font-weight: 600;">$${cost.toFixed(4)}</td>
                <td class="text-right" style="color: #d29922; font-weight: 600;">${formatIDR(cost)}</td>
            `;
            tableBody.appendChild(tr);
        });

        // Grand total summary row (always visible, based on all data not just current page)
        const totalRow = document.createElement('tr');
        totalRow.style.cssText = 'border-top: 2px solid var(--panel-border); font-weight: 700; background: rgba(255,255,255,0.03);';
        totalRow.innerHTML = `
            <td style="color: var(--text-muted); font-size: 0.8rem;">TOTAL</td>
            <td colspan="3" style="color: var(--text-muted); font-size: 0.8rem;">${data.length} baris${totalPages > 1 ? ' (semua halaman)' : ''}</td>
            <td class="text-right">${grandTotalTokens.toLocaleString()}</td>
            <td class="text-right" style="color: #8b949e;">${grandTotalInput.toLocaleString()}</td>
            <td class="text-right" style="color: #8b949e;">${grandTotalOutput.toLocaleString()}</td>
            <td class="text-right" style="color: #3fb950;">$${grandTotalCost.toFixed(4)}</td>
            <td class="text-right" style="color: #d29922;">${formatIDR(grandTotalCost)}</td>
        `;
    ...
    const renderTopCards = (data) => {
        ...
        topModels.forEach(([model, stats], index) => {
            ...
            card.innerHTML = `
                ...
                <span class="stat-value" title="${stats.tokens.toLocaleString()} tokens">${formattedTokens} <span style="font-size:1rem; color:#3fb950; font-weight: 500;">≈ $${cost.toFixed(2)}</span> <span style="font-size:0.85rem; color:#d29922;">(${formatIDR(cost)})</span></span>
            `;
    ...
    const renderChart = (data) => {
        ...
        // Handle all period formats: YYYY-MM-DD, YYYY-MM, YYYY-WW, YYYY-MM-DD HH:00, YYYY-MM-DD HH:MM
        const parsePeriod = (p) => {
            // Weekly format YYYY-WW — sort lexicographically (works because zero-padded)
            if (/^\d{4}-\d{2}$/.test(p) && parseInt(p.split('-')[1]) <= 53) return p; // monthly or weekly: sort as string
            return new Date(p).getTime() || p; // daily, hourly, minutely
        };
        const periods = [...new Set(data.map(d => d.period_date))].sort((a, b) => {
            const pa = parsePeriod(a);
            const pb = parsePeriod(b);
            if (typeof pa === 'string' && typeof pb === 'string') return pa.localeCompare(pb);
            return pa - pb;
        });
        ...
        // 3. Aggregate token_count and cost for each model per period across ALL modalities
        const datasets = models.map((model, index) => {
            const dataPoints = periods.map(period => {
                const records = data.filter(d => d.period_date === period && d.model === model);
                return records.reduce((sum, r) => sum + parseInt(r.total_tokens, 10), 0);
            });

            const costPoints = periods.map(period => {
                const records = data.filter(d => d.period_date === period && d.model === model);
                return records.reduce((sum, r) => {
                    return sum + calculateCost(model, parseInt(r.input_tokens || 0, 10), parseInt(r.output_tokens || 0, 10), r.modality);
                }, 0);
            });
    ...
    if (modalityFilter) {
        modalityFilter.addEventListener('change', fetchData);
    }
    if (timezoneFilter) {
    ...
    btnExportCSV.addEventListener('click', () => {
        ...
        const headers = ["Period", "Model", "Project", "Modality", "Input Tokens", "Output Tokens", "Total Tokens", "Est. Cost (USD)", `Est. Cost (IDR@${idrRate})`];
        const rows = currentData.map(row => {
            const cost = calculateCost(row.model, row.input_tokens || 0, row.output_tokens || 0, row.modality);
            const projLabel = projectLabels[row.project_id] || row.project_id || '';
            const idr = Math.round(cost * idrRate);
            return `"${row.period_date}","${row.model}","${projLabel}","${row.modality || ''}","${row.input_tokens || 0}","${row.output_tokens || 0}","${row.total_tokens}","${cost.toFixed(4)}","${idr}"`;
        });
    ...
    btnExportXLS.addEventListener('click', () => {
        ...
        const wsData = [
            ["Period", "Model", "Project", "Modality", "Input Tokens", "Output Tokens", "Total Tokens", "Est. Cost (USD)", `Est. Cost (IDR@${idrRate})`],
            ...currentData.map(row => {
                const cost = calculateCost(row.model, row.input_tokens || 0, row.output_tokens || 0, row.modality);
                const projLabel = projectLabels[row.project_id] || row.project_id || '';
                const idr = Math.round(cost * idrRate);
                return [row.period_date, row.model, projLabel, row.modality || '', row.input_tokens || 0, row.output_tokens || 0, row.total_tokens, Number(cost.toFixed(4)), idr];
            })
        ];
